# Chapter ingest — Understanding Distributed Systems (2e) · Chapter 14
...
| ingest_path | /Users/dubs/Projects/scholia.skill/literature/cs-ai-textbook-canon/ingests/understanding_distributed_systems_2022_ch14_ingest.md |
| text_lines_read | 4817–4963 |
...
Client-side HTTP caching (Cache-Control, ETag, 304) for static assets; immutable URL versioning; CQRS read/write split; reverse proxies as shared server-side cache bridging to CDN.
...
1. **Client cache flow** (4827–4879): GET/HEAD only; TTL + ETag; fresh vs stale conditional If-None-Match.
2. **Immutable deploy** (4886–4902): New URLs force fetch; atomic website version cutover.
3. **CQRS** (4904–4909): Optimize read path separately from writes.
4. **Reverse proxy** (4918–4955): Shared cache; auth, compress, rate-limit, LB; commoditized by CDN.

---

## Section digest (anchored)

### §14 client cache (4827–4909)

Figures 14.1–14.2; CQRS mention.

### §14.1 Reverse proxies (4918–4955)

Figure 14.3; NGINX/HAProxy.
...
1. Explain http caching role in Cruder scale path.
...
1. **Design hook:** Apply HTTP caching to one operator static-asset path.
...
|---------|--------|-------------|
| Ch.14 full slice | Read | 4817–4963 |

**Attestation:** Single-file read of `understanding_distributed_systems_2022.txt` lines 4817–4963 only.
...
First Part III read-path optimization on Cruder static assets.
...
- Claims trace to lines 4817–4963 of `understanding_distributed_systems_2022.txt`.
